...
def fetch_optimizer_and_scheduler(params, net):
    # 优化器设置，正则化
    log_to_console('Weight decay: ', params.weight_decay)
    ...
    if params.scheduler == 'step_lr_big_step':
        log_to_console('Using big step for learning rate decay')
    ...
    log_to_console('Learning rate: {}, optimizer: {}, scheduler: {}'.format(params.learning_rate, params.optimizer,
                                                                            params.scheduler))

    if params.warmup_lr:
        log_to_console('Start training with learning rate warmed up')
