...
from key_modules import loss_and_net, mydataloader
...
evaluate_parser.add_argument('-exp', '--experiment_dir', default='../experiments/exp10_learning_rate/scheduler/scheduler_step_cosineAnnWarm_T4',
                             type=str,
...
def evaluate_fundamental(net, evaluate_dataset, experiment_dir=None, use_train_set=True, loss_module=None,
    ...
    for num, i in enumerate(index):  # 在训练集上测试
        ...
        Groundtruth = Groundtruth/Groundtruth[-1,-1] # Fundamental matrix除以自身最后一个元素进行normalize
        p1, p2 = label[1:3]
        p1 = p1.T.copy()
        p2 = p2.copy()
    ...
    if save_metric:
        # 保存save metric，在ide中直接运行的时候默认是不保存的，只是看画图结果
        metric = {}
...
if __name__ == '__main__':
    ...
    if args.save_metric: # linux命令行中运行，保存结果文件
        ...
        sample_num_to_show = 2 # 4 figs per sample
        if use_train_set:
    ...
    if not use_train_set:  # 用测试集
        ...
        evaluate_set = train_set
        checkpoint = torch.load(experiment_dir + os.sep + 'checkpoint.pth', map_location='cpu')
        print('Use the newest net!')


    # --------------------------------兼容性设计----------------------------
    # 有时候修改了代码里模型的参数名字，但是checkpoint保存的net state dict还没有改
    net_dict = checkpoint['net_state_dict']
    temp_new_dict={}
    for key,value in net_dict.items():
        if 'concat_layer' in key:
            temp_new_dict.update({key.replace('concat_layer', 'concat_fc_layer'):value})
        else:
            temp_new_dict.update({key:value})
    net.load_state_dict(temp_new_dict) # 加载训练模型参数

    # --------------------------------------------------------------------

    print(checkpoint['log_label'], checkpoint['trained_epoch'])
    net.eval()  # 改为评估模式，去掉drop层,修改bn层的作用方式
    loss_module = loss_and_net.loss_reduce_epi
    evaluate_fundamental(net=net, evaluate_dataset=evaluate_set, experiment_dir=experiment_dir,
    ...
    #     print('Quaternion difference for gt and net output:\n',quat_gt-quat)
