...
def setup_seed(args, params):
    ...
    if params.cuda_seed:
        print('Set one cuda seed for all gpus! Probably degrades performance')
    ...
    if cudnn.benchmark:
        print("cudnn.deterministic: ",cudnn.deterministic)
        print('Cudnn benchmark has been turned on! Check out the performance difference')
...
def search_experiment(id:int,name:str,experiments_root=r'../experiments'):
    '''
    根据实验id和name来确定路径，如果存在实验id和name都match的文件夹，就返回路径，如果不存在实验id，则新建一个文件夹
    :param id:
    :param name:
    :param experiments_root:
    :return:
    '''
    ...
    if len(found_dir)>1: # 找到不止一个文件夹，那肯定有问题
        print(found_dir)
        raise ValueError('More than one experiment found by id! Rename them!')

    elif len(found_dir)==0: # 新的实验，创建文件夹
        ...
        found_exp_name = found_dir.replace(f'exp{id}_'.format(id=id),'') # 去掉exp id部分，获得名字
...
def find_all_experiment_dirs(parent_dir_path, experiments_list=None, find_file = None,exclude_file=None):
    ...
    '''
    if not exclude_file:
        exclude_file = ['metrics_train_set.json','metrics_test_set.json'] # 如果已经存在结果了，就不再evaluate
    ...
    check_experiment_dir = lambda dir_path: check_exists_file(dir_path) and not check_exclude_file(dir_path) # 存在checkpoint且不存在metric result
...
def backup_code(target_experiment_dir):
    # 所有实验完成了，记录实验中用到的代码进行改动

    code_backup_dir = os.path.join(target_experiment_dir, 'code_backup')
    if not os.path.isdir(code_backup_dir):
        os.mkdir(code_backup_dir)  # 创建备份文件夹
    print('Backup codes at :', code_backup_dir)
    for root, code_dirs, files in os.walk('./'):  # 广度优先遍历code文件夹
        if '__pycache__' in root or '.@__thumb' in root:
            continue
        for code_dir in code_dirs:
            if code_dir == '.@__thumb' or code_dir == '__pycache__':  # 跳过无用文件夹
                continue
...
def set_params_default(default_params:dict,parent_dir ='../experiments' ):
    '''
    遍历所有实验的params.json文件，设置某个键值对的默认值
...
def rename_param_name(old_key_name:str, new_key_name:str):
    experiments_list = []
    find_all_experiment_dirs(parent_dir_path='../experiments',experiments_list=experiments_list,find_file='params.json')
    for exp_path in experiments_list:
        params_path  = exp_path+os.sep+'params.json'
        params = Params(params_path)
